...
from in_the_wild.utils import read_video
...
def draw_3d_image(pred_all, skeleton, azim, video_name=""):
    for frame in range(pred_all.shape[2]):
        ...
        #for t in range(0,pred.shape[0]):
            ...
            for j, j_parent in enumerate(parents):
                mean_pose = np.mean(pred, axis=1, keepdims=False)
                col = 'red'
                t = 4
                if j_parent == -1:
                    ax.scatter(mean_pose[t, j, 0], mean_pose[t, j, 1], mean_pose[t, j, 2], c=col, s=1)
                    continue
                ...
                # for t in range(mean_pose.shape[0]):
                ax.plot([mean_pose[t, j, 0], mean_pose[t, j_parent, 0]],
                        [mean_pose[t, j, 1], mean_pose[t, j_parent, 1]],
                        [mean_pose[t, j, 2], mean_pose[t, j_parent, 2]], zdir='z', linewidth=0.5, c=col)
