...
# TODO: need to adapt this one for in the wild trainings!!! Currently it is not working for PAFUSE.
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        ...
        for cam, batch, batch_2d in test_generator.next_epoch():
            ...
            for batch_cnt in range(total_batch):
                ...
                error_h, _ = mpjpe_diffusion(predicted_3d_pos_single, inputs_3d_single)
                ...
                error_p2 = p_mpjpe_diffusion_all_min(predicted_3d_pos_single, inputs_3d_single) # J-Best
                error_h_p2 = p_mpjpe_diffusion(predicted_3d_pos_single, inputs_3d_single) # P-Best
                error_mean_p2 = p_mpjpe_diffusion_all_min(predicted_3d_pos_single, inputs_3d_single, mean_pos=True) # P-Agg
                error_reproj_select_p2 = p_mpjpe_diffusion_reproj(predicted_3d_pos_single, inputs_3d_single, reproject_2d, inputs_2d_single) # J-Agg
    ...
    log_path = os.path.join(args.checkpoint, 'test_log_H%d_K%d_in_the_wild.txt' %(args.num_proposals, args.sampling_timesteps))
    f = open(log_path, mode='a')
    if action is None:
        print('----------')
    else:
